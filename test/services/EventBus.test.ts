...
describe('EventBus', () => {
    ...
    it('removes the connection as a receiver if it detects a stale connection', async () => {
        ...
        expect(topicsRepository.unsubscribeFromAll).toHaveBeenCalledWith('connection-1');
    });
    ...
    it('throws the error if it receives a non-410 error when sending the event', async () => {
        ...
        await expect(eventBus.send('connection-1', event)).rejects.toThrowError('Unprocessable Entity');
    });
})
