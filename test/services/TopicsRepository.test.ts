...
describe("TopicsRepository", () => {
  ...
  it("can unsubscribe a connection from every topic", async () => {
    ddbMock.on(ScanCommand).resolves({
      Items: [
        {
          name: "topic-1",
          receivers: new Set(["connection-1", "connection-2"]),
        },
        {
          name: "topic-2",
          receivers: new Set(["connection-2", "connection-3"]),
        },
        {
          name: "topic-3",
          receivers: new Set(["connection-3", "connection-1"]),
        },
      ],
    });
  ...
  it("can return the receivers for a topic", async () => {
    ...
    expect(result).toEqual(["connection-1", "connection-2"]);
  });
});
